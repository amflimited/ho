...
if ($offer === 'receptionist') {
    // Receptionist track is keyed by business_slug (no preview row required).
    $biz = strtolower((string)preg_replace('/[^a-z0-9-]/i', '', (string)($_GET['biz'] ?? '')));
    $q = $pdo->prepare('SELECT id AS biz_id, business_name, business_slug FROM businesses WHERE business_slug = ?');
    $q->execute([$biz]);
...
if ($offer === 'receptionist') {
    $amount = max(1, (int)(ho_setting($pdo, 'rcpt_price_cents') ?: 14900));
    $params = [
        'mode' => 'subscription',
        'success_url' => $base . '/status.php?session={CHECKOUT_SESSION_ID}',
        'cancel_url'  => $base . '/listen/' . $row['business_slug'],
        'line_items[0][quantity]' => 1,
        'line_items[0][price_data][currency]' => 'usd',
        'line_items[0][price_data][unit_amount]' => $amount,
        'line_items[0][price_data][recurring][interval]' => 'month',
        'line_items[0][price_data][product_data][name]' => 'AI receptionist — ' . $row['business_name'],
        'metadata[business_id]' => (string)(int)$row['biz_id'],
        'metadata[package]' => 'receptionist',
        // Subscription sessions surface metadata on the subscription too.
        'subscription_data[metadata][business_id]' => (string)(int)$row['biz_id'],
    ];
} else {

...
function wmt_has($s,$needle){ return strpos((string)$s,(string)$needle) !== false; }
function wmt_mins($t){ if(!preg_match('/^(\d{1,2}):(\d{2})/',(string)$t,$m)) return 0; return ((int)$m[1]*60)+(int)$m[2]; }
function wmt_t($m){ $m=(($m%1440)+1440)%1440; return sprintf('%02d:%02d', intdiv($m,60), $m%60); }
function wmt_nice($m){ $m=(($m%1440)+1440)%1440; $h=intdiv($m,60); $ap=$h>=12?'PM':'AM'; $hh=$h%12; if(!$hh)$hh=12; return $hh.':'.sprintf('%02d',$m%60).$ap; }
function wmt_team($r){ $x=strtolower(($r['team']??'').' '.($r['role_type']??'')); if(wmt_has($x,'invest')) return 'Investigator'; if(wmt_has($x,'lead')||wmt_has($x,'tl')) return 'Team Lead'; if(wmt_has($x,'ops')||wmt_has($x,'operation')) return 'Ops'; return 'Services'; }
...
function wmt_seed($pdo){
  $settings=array('triage_start'=>'08:00','triage_end'=>'17:00','grocery_gold_start'=>'06:00','grocery_gold_end'=>'23:00','gm_gold_start'=>'06:00','gm_gold_end'=>'21:00','slot_minutes'=>'15','ops_blackout_start'=>'10:00','ops_blackout_end'=>'12:00','ops_flex_per_person'=>'30','tl_flex_per_day'=>'15','investigator_flex'=>'never');
...
function wmt_import_payload($pdo,$j){
  ...
  foreach($j['schedule']??array() as $s){
    ...
    $pdo->prepare('INSERT INTO wmt_shifts(work_date,associate_id,associate_name,team,role_type,start_time,end_time,notes) VALUES(?,?,?,?,?,?,?,?)')->execute(array($d,$id,$n,$s['team']??'Services',$s['role_type']??'AP Service TA',$start,$end,$s['notes']??null));
    $count++;
  }
  return 'Imported '.$count.' schedule rows';
}
function wmt_import_current_week($pdo){
  $file=__DIR__.'/current-week-import.json'; if(!is_file($file)) return 'Current-week import file not found.';
  $j=json_decode(file_get_contents($file),true); return wmt_import_payload($pdo,$j);
}
function wmt_auto_seed($pdo){
  if(wmt_get($pdo,'current_week_auto_seeded')==='1') return;
  $cnt=(int)$pdo->query('SELECT COUNT(*) FROM wmt_shifts')->fetchColumn();
  if($cnt===0 && is_file(__DIR__.'/current-week-import.json')){ wmt_import_current_week($pdo); wmt_set($pdo,'current_week_auto_seeded','1'); }
}
function wmt_post($pdo){
  if($_SERVER['REQUEST_METHOD']!=='POST') return '';
  $f=$_POST['form']??'';
  if($f==='current_week') return wmt_import_current_week($pdo);
  if($f==='json'){ $j=json_decode($_POST['payload']??'',true); return wmt_import_payload($pdo,$j); }
  if($f==='csv'){
    $lines=preg_split('/\r\n|\n|\r/',trim($_POST['csv']??'')); if(!$lines || count($lines)<2) return 'No CSV rows found.';
    $h=array_map('trim',str_getcsv(array_shift($lines))); $h=array_map('strtolower',$h); $c=0;
...
function wmt_breaks($s){
  $st=wmt_mins(substr($s['start_time'],0,5)); $en=wmt_mins(substr($s['end_time'],0,5)); if($en<=$st)$en+=1440; $dur=$en-$st; $out=array();
  if($dur>=480){ $q=$dur/4; $out[]=array('Break 1',round(($st+$q-8)/15)*15,15); $out[]=array('Lunch',round(($st+2*$q-30)/15)*15,60); $out[]=array('Break 2',round(($st+3*$q-8)/15)*15,15); }
  elseif($dur>=360){ $out[]=array('Break',round(($st+$dur/3-8)/15)*15,15); $out[]=array('Lunch',round(($st+$dur*2/3-15)/15)*15,30); }
  elseif($dur>=240){ $out[]=array('Break',round(($st+$dur/2-8)/15)*15,15); }
  return $out;
}
function wmt_off($s,$a,$b){ foreach(wmt_breaks($s) as $x){ if($a<$x[1]+$x[2] && $b>$x[1]) return $x[0]; } return ''; }
function wmt_on($s,$a,$b){ $st=wmt_mins(substr($s['start_time'],0,5)); $en=wmt_mins(substr($s['end_time'],0,5)); if($en<=$st)$en+=1440; return $a>=$st && $b<=$en; }
...
function wmt_plan($pdo,$d){
  $sh=wmt_shifts($pdo,$d); $rows=array(); $counts=array(); $opsUsed=array(); $tlUsed=0; $gap=0; $cov=0; $req=0; $float=0; $slot=(int)wmt_get($pdo,'slot_minutes',15); if($slot<5)$slot=15; $start=wmt_mins(wmt_get($pdo,'triage_start','08:00')); $end=wmt_mins(wmt_get($pdo,'triage_end','17:00')); $opsB=wmt_mins(wmt_get($pdo,'ops_blackout_start','10:00')); $opsE=wmt_mins(wmt_get($pdo,'ops_blackout_end','12:00')); $opsMax=(int)wmt_get($pdo,'ops_flex_per_person',30); $tlMax=(int)wmt_get($pdo,'tl_flex_per_day',15);
  for($t=$start;$t<$end;$t+=$slot){
    $u=array(); $off=array();
    foreach($sh as $s){ if(wmt_on($s,$t,$t+$slot)){ $o=wmt_off($s,$t,$t+$slot); if($o) $off[]=$s['associate_name'].' '.$o; else $u[]=$s; } }
  ...
  return array('date'=>$d,'shifts'=>$sh,'rows'=>$rows,'gap'=>$gap,'cov'=>$cov,'req'=>$req,'pct'=>$req?round($cov/$req*100,1):0,'float'=>$float,'opsUsed'=>$opsUsed,'tlUsed'=>$tlUsed);
}
...
try{
  ...
  if($v==='week') wmt_week($pdo); elseif($v==='import') wmt_import_page($msg); elseif($v==='turnin') wmt_turnin($pdo,$msg); elseif($v==='mgmt') wmt_mgmt($pdo); elseif($v==='ta') wmt_ta($pdo); else wmt_dashboard($pdo,$msg);

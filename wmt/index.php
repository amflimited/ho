...
function wmt_has($s,$n){ return strpos((string)$s,(string)$n) !== false; }
...
function wmt_seed($pdo){
  $settings=array('triage_start'=>'08:00','triage_end'=>'17:00','slot_minutes'=>'15','ops_blackout_start'=>'10:00','ops_blackout_end'=>'12:00','ops_flex_per_person'=>'30','tl_flex_per_day'=>'15','grocery_gold_start'=>'06:00','grocery_gold_end'=>'23:00','gm_gold_start'=>'06:00','gm_gold_end'=>'21:00','investigator_flex'=>'never');
  ...
  $q=$pdo->prepare('INSERT IGNORE INTO wmt_tasks(name,category,priority,weight,frequency) VALUES(?,?,?,?,?)');
  foreach($tasks as $t){ $q->execute($t); }
}
...
function wmt_up_assoc($pdo,$r){
  ...
  if(!$id){ $q=$pdo->prepare('SELECT id FROM wmt_associates WHERE name=?'); $q->execute(array($n)); $id=(int)$q->fetchColumn(); }
  return $id;
}
...
function wmt_shift_covers($s,$a,$b){ $x=wmt_shift_span($s); return $a>=$x[0] && $b<=$x[1]; }
function wmt_raw_breaks($s,$slot=15){
  $x=wmt_shift_span($s); $st=$x[0]; $en=$x[1]; $dur=$x[2]; $out=array();
...
function wmt_break_needs($s,$slot){
  $x=wmt_shift_span($s); $st=$x[0]; $en=$x[1]; $dur=$x[2]; $n=$s['associate_name']; $needs=array();
...
function wmt_build_break_plan($sh,$slot){
  $needs=array();
  foreach($sh as $s){ if(wmt_team($s)==='Services' && (int)($s['can_cover']??1)){ foreach(wmt_break_needs($s,$slot) as $b){ $needs[]=$b; } } }
  ...
  foreach($needs as $b){
    ...
    $best=$b['ideal']; $bestScore=999999999; $bestImpact=0;
    for($cand=$lo; $cand<=$hi; $cand+=$slot){
      $score=abs($cand-$b['ideal']); $impact=0;
      foreach($events as $p){ if($p['associate']===$b['associate']){ if($b['order']>$p['order'] && $cand < $p['end']+60) $score+=50000; if($b['order']<$p['order'] && $cand+$b['duration']+60 > $p['start']) $score+=50000; } }
      for($t=$cand; $t<$cand+$b['duration']; $t+=$slot){ $already=count($breakSlots[$t]??array()); if($already>0) $score+=5000*$already; $after=wmt_available_services($sh,$t,$t+$slot,$breakSlots,$b['associate']); if($after<=0){ $score+=100000; $impact+=2; } elseif($after==1){ $score+=900; $impact+=1; } else { $score-=200; } }
      if($score<$bestScore){ $bestScore=$score; $best=$cand; $bestImpact=$impact; }
    }
    $b['start']=$best; $b['end']=$best+$b['duration']; $b['impact']=$bestImpact;
    if($bestImpact>0) $warnings[]=$b['associate'].' '.$b['type'].' has coverage impact; no cleaner stagger window existed.';
    $events[]=$b;
...
function wmt_off_label($breakPlan,$s,$slot){
  ...
  if(wmt_team($s)==='Services') return $breakPlan['labels'][$name][$slot]??'';
...
function wmt_pick_service($c,$door,$used,$counts){ $best=null; $score=999999; foreach($c as $r){ $n=$r['associate_name']; if(isset($used[$n])) continue; $s=($counts[$n]??0)*10 - ((float)($r['reliability']??3))*2; $pref=$r['preferred_door']??'Either'; if($pref===$door) $s-=20; elseif($pref==='Either') $s-=5; if($s<$score){ $score=$s; $best=$r; } } return $best; }
function wmt_plan($pdo,$d){
  ...
  for($t=$start; $t<$end; $t+=$slot){
    ...
    foreach(array('Grocery','GM') as $door){
      ...
      if(count($ops)>=2 && !($t<$opsE && $t+$slot>$opsB)){ foreach($ops as $o){ $n=$o['associate_name']; if(!isset($used[$n]) && (($opsUsed[$n]??0)+$slot)<=$opsMax){ $assigned=$o; $used[$n]=1; $opsUsed[$n]=($opsUsed[$n]??0)+$slot; $flex[]=$n.' Ops flex'; break; } } }
